...
public class JWTTokenProvider {
    ...
    static public String getToken(String email,String nivel)
    {       
        String jwtToken = Jwts.builder()
            ...
            .setExpiration(Date.from(LocalDateTime.now().plusMinutes(1L) //token expira em 1minuto
    ...
    static public boolean verifyToken(String token)
    {
        try {
            Jwts.parserBuilder()
                .setSigningKey(CHAVE)
                .build()
                .parseClaimsJws(token);//.getSignature(); //comentei esse getsinature porque ta dando um b.o sinistro..
                return true;
       } catch (Exception e) {
